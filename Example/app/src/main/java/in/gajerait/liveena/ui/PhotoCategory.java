...
import in.gajerait.liveena.adapter.PhotoCategoryAdapter;
...
public class PhotoCategory extends AppCompatActivity implements ViewPagerEx.OnPageChangeListener {
    ArrayList<BeanPhotoCategory> AvailVoucher;
    private Activity activity;
    private RecyclerView PhotoCategory_recyclerView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        PhotoCategory_recyclerView = (RecyclerView) findViewById(R.id.PhotoCategory_recyclerView);
        PhotoCategory_recyclerView.setLayoutManager(new GridLayoutManager(activity, 2));
//        PhotoCategory_recyclerView.setLayoutManager(new LinearLayoutManager(activity));
    ...
    private void PhotoCategory() {
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, "http://liveena.com/image_category_api.php",
                        ...
                        try {
                            ...
                            if (success == 1) {
                                ...
                                if (jaDetail.length() > 0) {
                                    ...
                                    PhotoCategory_recyclerView.setVisibility(View.VISIBLE);
                                    adapter = new PhotoCategoryAdapter(activity, AvailVoucher, "avail");
                                    PhotoCategory_recyclerView.setAdapter(adapter);
                                    pDialog.dismiss();
                                } else {
                                    pDialog.dismiss();
                                    CommonUtility.showAlertDialog(activity, getResources().getString(R.string.No_Internet), getResources().getString(R.string.app_name), true);
